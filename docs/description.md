...
Подключение: оператор `"use", string, line break`; `string` &mdash;
относительный путь к библиотеке; имя библиотеки может указываться в сокращённом
варианте (`Name` вместо `libName.a`); если библиотека расположена по
стандартному пути, путь можно опустить; стандартные библиотеки языков C и C++
